<script setup>
import { onMounted, ref, computed } from "vue";
import { useRouter } from 'vue-router'
import dayjs from 'dayjs'
import QRcode from 'qrcode'
import progressBar from '@/components/progressBar.vue';
import { getOrderDetail, createOrder } from '@/api/order';
const router = useRouter()
const orderInfo = ref({
    companion: [],
    hospitals: [],
    service: {}
})
onMounted(async () => {
    const { data } = await getOrderDetail();
    if (data.code === 10000) {
        Object.assign(orderInfo.value, data.data);
    }
});
const form = ref({
    hospital_id: '',
    hospital_name: '',
    demand: '',
    companion_id: '',
    receiveAddress: '',
    tel: '',
    starttime: 0,
})
//选择医院
const showHosp = ref(false)
const showHospColumns = computed(() => {
    return orderInfo.value.hospitals.map(item => {
        return { text: item.name, value: item.id }
    })
})
const onConfirmHosp = (item) => {
    // console.log(item)
    form.value.hospital_id = item.selectedOptions[0].value
    form.value.hospital_name = item.selectedOptions[0].text
    showHosp.value = false

}
//选择日期
const showTime = ref(false)
const minDate = new Date()
const currentDate = ref()
const confirmTime = (item) => {
    const dataStr = item.selectedValues.join('-')
    form.value.starttime = dayjs(dataStr).unix()
    // console.log(form.value.starttime)
    showTime.value = false
}
//选择陪诊师
const showMan = ref(false)
const companionName = ref()
const showManColumns = computed(() => {
    return orderInfo.value.companion.map(item => {
        return { text: item.name, value: item.id }
    })
})
const onConfirmMan = (item) => {
    form.value.companion_id = item.selectedOptions[0].value
    companionName.value = item.selectedOptions[0].text
    showMan.value = false
}
const showcode = ref(false)
const code = ref()
const closeCode = async() => { 
    showcode.value = false
    router.push('/order')
}
import { showNotify } from 'vant';
const submitOrder = async () => {
    const params = [
        'hospital_id',
        'hospital_name',
        'demand',
        'companion_id',
        'receiveAddress',
        'tel',
        'starttime'
    ]
    for (const i of params) {
        if (!form.value[i]) {
            showNotify({ message: '请填写每一项' });
            return
        }
    }
    const { data } = await createOrder(form.value)
    if (data.code === 10000) {
        QRcode.toDataURL(data.data.wx_code).then(url => { 
            code.value = url
            showcode.value = true
        })

    }
}    
</script>

<template>
    <div class="container">
        <div class="header">
            <van-icon @click="router.go(-1)" name="arrow-left" class="header-left" size="30" />
            填写服务订单
        </div>
        <progressBar :item="0"></progressBar>
        <van-cell class="cell" style="height: 50px;">
            <template #title>
                <van-image width="30" height="30" :src="orderInfo.service.serviceImg"></van-image>
                <span class="server-name"> {{ orderInfo.service.serviceName }} </span>
            </template>
            <template #default>
                <div class="service-text">服务内容</div>
            </template>
        </van-cell>
        <van-cell-group class="cell">
            <van-cell>
                <template #title>就诊医院</template>
                <template #default>
                    <div @click="showHosp = true">
                        {{ form.hospital_name ? form.hospital_name : '请选择就诊医院' }}
                        <van-icon name="arrow" />
                    </div>
                </template>
            </van-cell>
            <van-cell>
                <template #title>就诊时间</template>
                <template #default>
                    <div @click="showTime = true">
                        {{ currentDate ? currentDate.join('-') : '请选择就诊时间' }}
                        <van-icon name="arrow" />
                    </div>
                </template>
            </van-cell>
            <van-cell>
                <template #title>陪诊师</template>
                <template #default>
                    <div @click="showMan = true">
                        {{ companionName ? companionName : '请选择陪诊师' }}
                        <van-icon name="arrow" />
                    </div>
                </template>
            </van-cell>
            <van-cell class="cell-address">
                <van-field v-model="form.receiveAddress" label="接送地址" placeholder="请填写就诊人地址" input-align="right" />
            </van-cell>
            <van-cell class="cell-phone">
                <van-field v-model="form.tel" label="联系电话" placeholder="请填写就诊人联系电话" input-align="right" />
            </van-cell>
        </van-cell-group>
        <van-cell-group class="cell" title="服务需求">
            <van-field v-model="form.demand" placeholder="请简单描述您要就诊的科室" style="height: 100px;" />
        </van-cell-group>

        <van-button type="primary" size="large" class="submit" @click="submitOrder">提交订单</van-button>
        <van-popup v-model:show="showHosp" position="bottom" :style="{ height: '30%' }">
            <van-picker :columns="showHospColumns" @confirm="onConfirmHosp" @cancel="showHosp = false" />" />
        </van-popup>
        <van-popup v-model:show="showTime" position="bottom" :style="{ height: '30%' }">
            <van-date-picker v-model="currentDate" type="date" title="选择日期" :min-date="minDate"
                @cancel="showTime = false" @confirm="confirmTime" />" />
        </van-popup>
        <van-popup v-model:show="showMan" position="bottom" :style="{ height: '30%' }">
            <van-picker :columns="showManColumns" @confirm="onConfirmMan" @cancel="showMan = false" />
        </van-popup>

        <van-dialog :show-confirm-button="false" v-model:show="showcode">
            <van-icon name="cross" @click="closeCode" class="close" />
            <div>微信支付</div>
            <van-image height="150" width="150" :src="code" />
            <div>请使用本人微信扫描二维码</div>
        </van-dialog>
    </div>
</template>

<style lang="less" scoped>
.container {
    background-color: #f0f0f0;
    height: 100vh;
}

.header {
    padding: 10px 0;
    text-align: center;
    line-height: 30px;
    font-size: 15px;

    .header-left {
        float: left;
    }
}

.cell {
    width: 95%;
    margin: 5px auto;
    background-color: #fff;

    ::v-deep(.van-field__control) {
        color: var(--van-cell-value-color);
    }
}

.server-name {
    position: absolute;
    top: 12px;
    margin-left: 12px;
    font-size: 16px;
}

.service-text {
    background: url(data:image/png;base64,) no-repeat center center;
    background-size: 20px;
}

.submit {
    position: absolute;
    bottom: 0;
}

::v-deep(.van-dialog__content) {
    text-align: center;
    padding: 20px;

    .close {
        position: absolute;
        left: 20px;
    }
}

.cell-address {
    padding: 0 10px 0 0;
}

.cell-phone {
    padding: 20px 10px 10px 0;
}
</style>